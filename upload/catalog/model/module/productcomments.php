	...
	public function addComment($comment) {
		...
		if ($res = $this->db->query($sql)) {
        	if (filter_var($this->config->get('pcconf_email'), FILTER_VALIDATE_EMAIL)) {
				...
				$message = sprintf($this->language->get('pc_mail'), "<a href='" . $this->url->link('catalog/product', 'product_id=' . $prod['product_id'], 'SSL') . "'>$product_name</a>", $comment['name'], $comment['email'], nl2br($comment['comment']), HTTPS_SERVER . $this->language->get('pc_admin') . '/index.php?route=module/productcomments');
